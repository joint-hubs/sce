...
def test_mean_aggregation():
    ...
    df = pd.DataFrame({"city": ["A", "A", "B", "B"], "price": [100, 200, 300, 400]})
...
def test_std_aggregation():
    ...
    df = pd.DataFrame({"city": ["A", "A", "A"], "price": [100, 200, 300]})
...
def test_min_group_size_filter():
    ...
    df = pd.DataFrame(
        {"city": ["A", "A", "B", "C", "C", "C"], "price": [100, 110, 200, 300, 310, 320]}
    )
...
def test_relative_features_computation(sample_data):
    ...
    for col in ["price_mean", "price_std", "price_median"]:
        enriched[f"city_{col}"] = enriched["city"].map(city_stats.set_index("city")[col])

    # Compute relative features
    result = compute_relative_features(
        df=enriched, stats_dict=stats_dict, categorical_cols=["city"], target_col="price"
    )
...
def test_aggregation_with_missing_columns():
    ...
    df = pd.DataFrame({"city": ["A", "B"], "price": [100, 200]})
...
def test_aggregation_with_nans():
    ...
    df = pd.DataFrame({"city": ["A", "A", "A", "B", "B"], "price": [100, np.nan, 300, 200, 400]})
...
def test_count_aggregation():
    ...
    df = pd.DataFrame({"city": ["A", "A", "A", "B", "B"], "price": [100, 200, 300, 400, 500]})
...
def test_quantile_stats_added():
    ...
    df = pd.DataFrame({"city": ["A"] * 10, "price": list(range(100, 200, 10))})
...
def test_hierarchical_backoff_fills_nan():
    ...
    result = apply_hierarchical_backoff(
        df=df, stats_dict=stats_dict, categorical_cols=["city", "neighborhood"], target_col="price"
    )
...
def test_hierarchical_backoff_falls_back_to_global():
    ...
    result = apply_hierarchical_backoff(
        df=df, stats_dict=stats_dict, categorical_cols=["city"], target_col="price"
    )
...
def test_optimized_relative_features():
    ...
    result = compute_relative_features(
        df=df, stats_dict=stats_dict, hierarchy=["city"], target_col="price"
    )
