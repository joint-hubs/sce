...
class StatsAggregator:
    ...
    def aggregate(
        self, df: pd.DataFrame, group_cols: List[str], value_col: str, min_size: int = 5
    ) -> pd.DataFrame:
        """
        ...
        if AggregationMethod.COUNT in self.methods and count_col in stats.columns:
            ...
            if filtered_groups < initial_groups:
                logger.debug(
                    f"  Filtered to {filtered_groups} groups (min_size={min_size}), "
...
def compute_aggregations(
    ...
    logger.debug(
        f"min_group_size={min_group_size}, include_global={include_global}, "
    ...
    for cat_col in all_categoricals:
        ...
        if level_name not in stats_dict:
            try:
                ...
                logger.debug(
                    f"  '{cat_col}': {len(stats_df)} groups, {len(stats_df.columns)} features"
                )
...
def compute_relative_features(
    ...
    if hierarchy is not None and categorical_cols is None:
        warnings.warn(
            "hierarchy parameter is deprecated. Use categorical_cols.", DeprecationWarning
        )
    ...
    for level_name in stats_dict.keys():
        ...
        if mean_col in df.columns and std_col in df.columns:
            ...
            new_cols[f"{level_name}_zscore"] = (target_vals - mean_vals) / (std_vals + epsilon)

        # Ratio to median (Eq. 3): y / (median + ε)
        if median_col in df.columns:
            median_vals = df[median_col].values
            new_cols[f"{level_name}_ratio"] = target_vals / (median_vals + epsilon)
        ...
        if q25_col in df.columns and q75_col in df.columns:
            ...
            new_cols[f"{level_name}_pct_position"] = (target_vals - q25_vals) / (iqr_vals + epsilon)
