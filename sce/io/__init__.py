...
def _dataset_info_from_config(config_path: Path) -> DatasetInfo:
	...
	remote_source = None if manifest_entry is None or manifest_entry["source"] == "local" else manifest_entry["source"]
...
def load_dataset(dataset: str | Path, force_download: bool = False, **read_kwargs: Any) -> pd.DataFrame:
