...
class FeatureCombinationSearch:
    ...
    def _generate_combinations(self) -> List[Tuple[str, Tuple[str, ...], Tuple[str, ...]]]:
        ...
        # =====================================================================
        if n_context > 0:
            combinations.append(('base_context_all', tuple(self.base_features), tuple(self.context_features)))
            ...
            for model_cfg in self.model_configs:
                try:
                    ...
                    logging.getLogger(__name__).debug(
                        "Config %s strategy=%s model=%s failed: %s", config_id, strategy, model_cfg, e
                    )
    ...
    def _generate_ablation_combinations(
        ...
        try:
            ...
            for n_remove in [1, 2, 3, 5, 10]:
                ...
                combinations.append((
                    f'ablation_remove_best_{n_remove}',
                    base_remaining,
                    ctx_remaining
                ))
            ...
            for n_remove in [1, 2, 3, 5, 10]:
                ...
                ctx_remaining = tuple(f for f in remaining if f in self.context_features)
                combinations.append((
                    f'ablation_remove_worst_{n_remove}',
                    base_remaining,
                    ctx_remaining
                ))
